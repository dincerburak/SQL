...
SELECT customer.first_name,COUNT(*) FROM customer INNER JOIN payment ON customer.customer_id=payment.customer_id GROUP BY customer.first_name ORDER BY COUNT(*) DESC LIMIT 1;

--4=>category tablosundan kategori isimlerini ve kategori başına düşen film sayılarını sıralayınız.
SELECT name,COUNT(*) FROM category INNER JOIN film_category ON category.category_id=film_category.category_id GROUP BY category.category_id ORDER BY COUNT(*) DESC;
